use anyhow::{Context, Result};
use commonware_codec::{Encode, ReadExt};
use commonware_cryptography::ed25519::PrivateKey;
use commonware_cryptography::Signer;
use nullspace_types::{
    api::Submission,
    casino::{GameType, GlobalTableConfig},
    execution::{Instruction, Transaction},
};
use reqwest::Client;
use serde::Deserialize;
use std::time::{Duration, Instant};
use tokio::time::sleep;

const ADMIN_PRIV_HEX: &str = "b09e8077db829b08141f280fd3f25fdb4aec39adeb95e836081704896aa03bac";
const DEFAULT_BASE_URL: &str = "https://indexer.testnet.regenesis.dev";

#[derive(Deserialize)]
struct AccountResp {
    nonce: u64,
    balance: u64,
}

fn load_admin_key() -> Result<PrivateKey> {
    let bytes = commonware_utils::from_hex(ADMIN_PRIV_HEX)
        .context("decode admin hex")?;
    let mut slice = bytes.as_slice();
    let key = PrivateKey::read(&mut slice)?;
    Ok(key)
}

fn base_url() -> String {
    std::env::var("BASE_URL").unwrap_or_else(|_| DEFAULT_BASE_URL.to_string())
}

async fn fetch_nonce(client: &Client, base_url: &str, pub_hex: &str) -> Result<u64> {
    let account: AccountResp = client
        .get(format!("{base_url}/account/{pub_hex}"))
        .send()
        .await?
        .error_for_status()?
        .json()
        .await?;
    Ok(account.nonce)
}

async fn submit_tx(client: &Client, base_url: &str, tx: Transaction) -> Result<()> {
    let submission = Submission::Transactions(vec![tx]);
    let body = submission.encode().to_vec();
    let resp = client
        .post(format!("{base_url}/submit"))
        .header("Content-Type", "application/octet-stream")
        .body(body)
        .send()
        .await?;
    let status = resp.status();
    if !status.is_success() {
        let body = resp.text().await.unwrap_or_default();
        anyhow::bail!("submit failed: {} {}", status, body);
    }
    Ok(())
}

async fn wait_for_nonce(
    client: &Client,
    base_url: &str,
    pub_hex: &str,
    expected: u64,
) -> Result<()> {
    let timeout = Duration::from_secs(30);
    let start = Instant::now();
    loop {
        let nonce = fetch_nonce(client, base_url, pub_hex).await?;
        if nonce >= expected {
            return Ok(());
        }
        if start.elapsed() > timeout {
            anyhow::bail!("timeout waiting for nonce >= {expected} (got {nonce})");
        }
        sleep(Duration::from_millis(500)).await;
    }
}

#[tokio::main]
async fn main() -> Result<()> {
    let admin_priv = load_admin_key()?;
    let admin_pub = admin_priv.public_key();
    let pub_hex = commonware_utils::hex(admin_pub.encode().as_ref());
    let client = Client::builder().build()?;
    let base_url = base_url();

    let games = [
        GameType::Baccarat,
        GameType::Blackjack,
        GameType::CasinoWar,
        GameType::Craps,
        GameType::VideoPoker,
        GameType::HiLo,
        GameType::Roulette,
        GameType::SicBo,
        GameType::ThreeCard,
        GameType::UltimateHoldem,
    ];

    println!("Bootstrapping tables sequentially for admin {pub_hex}...");

    for game in games {
        let current_nonce = fetch_nonce(&client, &base_url, &pub_hex).await?;
        let cfg = GlobalTableConfig {
            game_type: game,
            betting_ms: 20_000,
            lock_ms: 2_000,
            payout_ms: 4_000,
            cooldown_ms: 4_000,
            min_bet: 5,
            max_bet: 1_000,
            max_bets_per_round: 100,
        };
        let init_tx = Transaction::sign(
            &admin_priv,
            current_nonce,
            Instruction::GlobalTableInit { config: cfg },
        );
        println!("Submitting {:?} init (nonce {})", game, current_nonce);
        submit_tx(&client, &base_url, init_tx).await?;
        wait_for_nonce(&client, &base_url, &pub_hex, current_nonce + 1).await?;

        let next_nonce = current_nonce + 1;
        let open_tx = Transaction::sign(
            &admin_priv,
            next_nonce,
            Instruction::GlobalTableOpenRound { game_type: game },
        );
        println!("Submitting {:?} open round (nonce {})", game, next_nonce);
        submit_tx(&client, &base_url, open_tx).await?;
        wait_for_nonce(&client, &base_url, &pub_hex, next_nonce + 1).await?;
    }

    println!("Bootstrap complete. Final nonce: {}", fetch_nonce(&client, &base_url, &pub_hex).await?);
    Ok(())
}
