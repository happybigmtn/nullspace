...
use nullspace_types::{
    ...
    casino::{GameType, GameSession, SuperModeState},
    NAMESPACE,
};
...
impl Context {
    async fn new() -> Self {
        ...
        ctx.execute(Instruction::CasinoDeposit { amount: 1_000_000 }).await;
    ...
    async fn inject_session(&mut self, session: GameSession) {
        self.state.insert(Key::CasinoSession(session.id), Value::CasinoSession(session)).await;
    }
...
async fn test_baccarat(ctx: &mut Context) {
    ...
    for (bet_type, name) in [(0, "Player"), (1, "Banker"), (2, "Tie")] {
        ...
        // 1. Start Game
        let events = ctx.execute(Instruction::CasinoStartGame {
        ...
        let events = ctx.execute(Instruction::CasinoGameMove {
            session_id,
            payload,
        }).await;
...
async fn test_craps(ctx: &mut Context) {
    ...
    // Test all bet types
    let bets = [
        (0, "Pass"), (1, "DontPass"), (2, "Come"), (3, "DontCome"), 
        (4, "Field"), (5, "Yes(4)"), (6, "No(4)"), (7, "Next(7)"),
        (8, "Hard4"), (9, "Hard6"), (10, "Hard8"), (11, "Hard10")
    ];
    ...
    if let Some(last) = events.last() {
        match last {
            Event::CasinoGameCompleted { payout, .. } => {
                let outcome = if *payout > 0 { "WIN" } else if *payout < 0 { "LOSS" } else { "PUSH" };
