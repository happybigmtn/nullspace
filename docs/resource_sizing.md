...
- Validators: 3x 4 vCPU, 8 GB RAM (consensus + execution).
...
- Validators: 3x 4-8 vCPU, 8-16 GB RAM (consensus + execution).
...
- Validators: 4-5x 8 vCPU, 16 GB RAM (consensus + execution).
...
- Mempool size (`mempool_max_transactions`) can be large; validate RAM use under realistic tx sizes.
  For the 5k target, reserve ~1-2 GiB RAM for mempool + overhead and reduce the
  cap if memory pressure appears.
