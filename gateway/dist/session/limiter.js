...
export class ConnectionLimiter {
    ...
    registerConnection(ip, connectionId) {
        ...
        console.log(`[Limiter] Connection registered: ${connectionId} from ${normalizedIp} ` +
     ...
     */
    unregisterConnection(ip, connectionId) {
        const normalizedIp = this.normalizeIp(ip);
        const ipConnections = this.connectionsByIp.get(normalizedIp);
        if (ipConnections) {
            if (ipConnections.delete(connectionId)) {
                ...
                console.log(`[Limiter] Connection unregistered: ${connectionId} from ${normalizedIp} ` +
                    `(Total: ${this.totalConnections}/${this.config.maxTotalSessions})`);
            }
        }
    }
