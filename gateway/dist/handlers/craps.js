...
import { encodeCrapsAtomicBatch } from '@nullspace/protocol/encode';
export class CrapsHandler extends GameHandler {
    ...
    async handleBet(ctx, msg) {
        ...
        const normalizeType = (value) => (typeof value === 'string' ? value.toUpperCase() : value);
        const bets = msg.type === 'craps_bet'
            ? [{
        ...
        try {
            const payload = encodeCrapsAtomicBatch(bets);
