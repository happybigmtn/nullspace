...
struct Args {
    ...
    /// Skip writing .env.local (useful for sandboxed or policy-restricted environments)
...
fn run() -> Result<()> {
    ...
    for i in 0..n {
        ...
        let bootstrapper = if n == 1 {
            "bootstrappers: []".to_string()
        } else if i == 0 {
...
"#,
    ...
    // Write identity to a plain text file (no env format) for tooling convenience.
    let identity_path = args.output.join("identity.hex");
    fs::write(&identity_path, &identity_hex)
        .with_context(|| format!("Failed to write identity {}", identity_path.display()))?;
    ...
    if !args.no_env {
        ...
        println!("Skipped writing .env.local (--no-env)");
    }
    ...
    if !args.no_env {
        ...
        println!("1. Export frontend env (example):");
        println!("   VITE_IDENTITY={} VITE_URL={} cd website && npm run dev", identity_hex, args.indexer);
        println!();
    }
