...
export function extractSecretValue(payload: string): string | null {
...
export function getMemberTiers(raw: string): string[] {
  return raw
    .split(',')
    .map((tier) => tier.trim())
    .filter(Boolean);
...
export function hasActiveEntitlement(entitlements: Entitlement[], tiers: string[]): boolean {
