...
const hasActiveEntitlement = (entitlements, tiers) => {
    return entitlements.some((entitlement) => {
        if (!ACTIVE_STATUSES.has(entitlement.status ?? ""))
            return false;
        if (tiers.length === 0)
            return true;
...
let nonceStorePromise = null;
const loadWasm = async () => {
    if (!wasmPromise) {
        wasmPromise = (async () => {
            const wasmUrl = new URL("../../../website/wasm/pkg/nullspace_wasm.js", import.meta.url);
            const wasmBinUrl = new URL("../../../website/wasm/pkg/nullspace_wasm_bg.wasm", import.meta.url);
...
const getNonceStore = async (state) => {
...
const enqueueAdmin = async (task) => {
    const next = adminQueue.then(task, task);
    adminQueue = next.then(() => undefined, () => undefined);
    return next;
};
const queryState = async (state, keyBytes) => {
...
const getAccountNonce = async (state) => {
...
const reserveNonce = async (state) => {
...
const resetNonceStore = async (state) => {
...
const getPlayer = async (state, publicKeyBytes) => {
...
const submitTransaction = async (state, tx) => {
...
export const syncFreerollLimit = async (publicKeyHex, entitlements) => {
    ...
    return enqueueAdmin(async () => {
        ...
        try {
            ...
            const tx = state.wasm.Transaction.casino_set_tournament_limit(state.signer, BigInt(nonce), playerKeyBytes, desiredLimit);
