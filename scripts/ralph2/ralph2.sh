...
# Usage: ralph2.sh [iterations] [-v|--verbose]
MAX_ITERATIONS=10
VERBOSE=false
...
echo -e "${BOLD}🔬 Starting Ralph2 - E2E QA Testing${NC}"
...
for i in $(seq 1 $MAX_ITERATIONS); do
  echo -e "${BOLD}═══ QA Iteration $i/$MAX_ITERATIONS ═══${NC}"

  if $VERBOSE; then
    # Verbose: show raw stream-json output
    claude --dangerously-skip-permissions --verbose \
      --output-format stream-json \
      -p "$(cat "$SCRIPT_DIR/prompt2.md")" 2>&1 \
      | tee "$RAWFILE" || true
  else
  ...
  if grep -q "<promise>QA-COMPLETE</promise>" "$RAWFILE"; then
    echo -e "${GREEN}✅ All QA tests passed!${NC}"
    exit 0
  fi

  if grep -q "<promise>INFRA-BLOCKED</promise>" "$RAWFILE"; then
    echo -e "${YELLOW}⚠️ Infrastructure blocked - check local network${NC}"
    exit 1
  fi
