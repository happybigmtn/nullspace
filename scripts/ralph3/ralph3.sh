...
# Usage: ralph3.sh [iterations] [-v|--verbose]
MAX_ITERATIONS=10
VERBOSE=false
...
echo -e "${BOLD}🎨 Starting Ralph3 - Design System Implementation${NC}"
...
for i in $(seq 1 $MAX_ITERATIONS); do
  echo -e "${BOLD}═══ Implementation Iteration $i/$MAX_ITERATIONS ═══${NC}"

  if $VERBOSE; then
    # Verbose: show raw stream-json output
    claude --dangerously-skip-permissions --verbose \
      --output-format stream-json \
      -p "$(cat "$SCRIPT_DIR/prompt3.md")" 2>&1 \
      | tee "$RAWFILE" || true
  else
  ...
  if grep -q "<promise>DESIGN-SYSTEM-COMPLETE</promise>" "$RAWFILE"; then
    echo -e "${GREEN}✅ Design system implementation complete!${NC}"
    exit 0
  fi

  if grep -q "<promise>BLOCKED:" "$RAWFILE"; then
    echo -e "${YELLOW}⚠️ Implementation blocked - check error details${NC}"
    exit 1
  fi
