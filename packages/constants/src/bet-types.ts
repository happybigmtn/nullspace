...
const CRAPS_HARDWAY_MAP: Record<number, number> = {
  4: 8,
  6: 9,
  8: 10,
  10: 11,
...
export function encodeCrapsBet(
  type: CrapsBetName,
  target?: number
): { betType: number; target: number } {
  let betType: number = CRAPS_BET_TYPES[type];
  let encodedTarget = target ?? 0;

  if (type === 'HARDWAY') {
    const hardwayBetType = target !== undefined ? CRAPS_HARDWAY_MAP[target] : undefined;
    betType = hardwayBetType ?? betType;
    encodedTarget = 0;
  }
...
export function crapsRequiresTarget(type: CrapsBetName): boolean {
...
export function rouletteRequiresTarget(type: RouletteBetName): boolean {
...
export function encodeSicBoBet(
  type: SicBoBetName,
  target?: number
): { betType: number; target: number } {
  return {
...
export function sicboRequiresTarget(type: SicBoBetName): boolean {
