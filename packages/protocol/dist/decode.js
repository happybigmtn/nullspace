...
export function decodeCard(data, offset) {
...
export function decodeCards(data, count, startOffset) {
    const expectedBytes = count * 3;
    if (startOffset + expectedBytes > data.length) {
        throw new ProtocolError(`Invalid cards data: expected ${expectedBytes} bytes at offset ${startOffset}, got ${data.length - startOffset}`);
    }
    const cards = [];
...
export function decodeGameResult(data) {
...
export function decodeBlackjackState(data) {
    ...
    const dealerCardCount = view.getUint8(offset);
    offset += 1;
    ...
    const playerTotal = view.getUint8(offset);
    offset += 1;
    const dealerTotal = view.getUint8(offset);
    offset += 1;
    const stageByte = view.getUint8(offset);
    offset += 1;
