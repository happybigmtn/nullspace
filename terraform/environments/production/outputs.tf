...
output "convex_servers" {
  description = "Convex server details"
  value       = var.enable_convex ? module.convex.servers : {}
}
...
output "simulator_lb_ip" {
  description = "Public IP of the simulator load balancer"
  value       = module.lb_simulator.public_ipv4
}

output "dns_records" {
  description = "Required DNS records for production"
  value = {
    "gateway.nullspace.example" = module.lb_gateway.public_ipv4
    "www.nullspace.example"     = module.lb_web.public_ipv4
    "api.nullspace.example"     = module.lb_web.public_ipv4
    "sim.nullspace.example"     = module.lb_simulator.public_ipv4
  }
}
