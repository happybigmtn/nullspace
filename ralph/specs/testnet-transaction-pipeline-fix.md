...
- React `#root` element exists but empty
- No console errors
- All resources load successfully

**Hypothesis**: Silent failure during app initialization, possibly in:
- WASM initialization
- WebSocket connection setup
- Initial seed/state fetch handling

### 5.2 Zero Transaction Blocks

**Symptoms**:
- Chain height increasing (blocks being produced)
- tx_count=0 in every block
- Validators running and connected

**Hypothesis**:
- Browser can't submit transactions (website not rendering)
- Or transactions are submitted but not reaching mempool
- Or mempool transactions not being included by proposers
...
- [ ] Submit transaction from browser
- [ ] Transaction appears in mempool
- [ ] Transaction included in block within 3 blocks
- [ ] Explorer shows transaction
- [ ] Balance updates correctly

### 8.3 Recovery Tests
- [ ] After chain reset, first transaction succeeds
- [ ] After container restart, transactions resume
- [ ] After browser refresh, nonce syncs correctly
