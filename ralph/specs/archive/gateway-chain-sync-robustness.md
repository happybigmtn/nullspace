...
**Status**: **Partially Complete** (BufferedMempool implemented, Explorer Persistence pending)
**Date**: 2026-01-16 → 2026-01-17
...
## 11. Related Issue: Explorer Data Persistence Gap

**Status**: Identified during investigation

### 11.1 Problem

The explorer shows 0 blocks after simulator restart because:
1. **Explorer state is in-memory only** - No persistence configured
2. **Validators track upload cursor** - They don't re-upload summaries already marked as uploaded
3. **Proofs are pruned after upload** - Old proofs are removed from validator cache after successful upload

When simulator restarts:
- Explorer state is lost (0 blocks)
- Validators have cursor at high height (e.g., 166057)
- Validators can't re-upload old summaries because proofs were pruned
- Explorer stays empty until consensus recovery

### 11.2 Immediate Workaround

Run consensus recovery to wipe validator state:
```bash
CONFIRM_RESET=1 ./scripts/testnet-consensus-recover.sh
```

This forces validators to restart from height 1 with fresh state.

### 11.3 Long-term Fix

Enable explorer persistence on the simulator:

```yaml
# In simulator.env or docker-compose
simulator:
  environment:
    # SQLite (simpler, single-node)
    - "--explorer-persistence-path=/data/explorer.db"
    # OR Postgres (better for multi-node or HA)
    - "--explorer-persistence-url=postgres://user:pass@host:5432/explorer"
```

With persistence enabled:
- Explorer data survives restarts
- Blocks are loaded from database on startup
- New blocks are persisted as they arrive

### 11.4 Acceptance Criteria for Explorer Persistence

- **AC-11.1**: Explorer data survives simulator restart
- **AC-11.2**: Explorer loads historical blocks on startup
- **AC-11.3**: New blocks are persisted within 1 second of indexing
- **AC-11.4**: Explorer pruning respects max_blocks limit
