...
**Goal**: Update **only** `IMPLEMENTATION_PLAN.md` to include ONLY tasks relevant to the provided work scope, and ensure every task is grounded in existing `specs/sprint2/*.md` and `specs/*.md` acceptance criteria (excluding `specs/archive/`).

### Scope Lock (Non-Negotiable)
- You may edit: `IMPLEMENTATION_PLAN.md` only.
- You may read: `specs/sprint2/*.md`, `specs/*.md` (excluding `specs/archive/`), and any repo files needed to confirm reality.
...
1. Identify which existing `specs/sprint2/*.md` and `specs/*.md` (excluding `specs/archive/`) files and ACs apply to the work scope.
...
3. Add/update tasks for the scope only, each with:
   - `Specs: specs/sprint2/<file>.md` or `specs/<file>.md` AC-X.Y
...
- [ ] Task description (clear, actionable)
  - Specs: `specs/sprint2/<file>.md` AC-X.Y, AC-A.B
  - Tests/backpressure:
    - Programmatic: ...
    - Programmatic: ...
