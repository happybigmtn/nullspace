...
- [x] Generate JS/TS bindings from Rust and remove hand-maintained payload encoders.
  - Tests: AC-3.1, AC-3.2, AC-4.2
  - Perceptual: None
  - Implemented: Extended `golden_vectors.rs` with 30+ game v2 compact encoding vectors covering all 10 games (Blackjack, Baccarat, Roulette, Craps, Sic Bo, Three Card, Ultimate Hold'em, Casino War, Video Poker, Hi-Lo); bumped `GOLDEN_VECTORS_SCHEMA_VERSION` to 2; `export_protocol --golden-vectors` CLI now exports game vectors for JS/TS consumption; added parity tests `test_blackjack_hit_parity_ac_3_2`, `test_roulette_straight_bet_parity_ac_3_2`, `test_craps_pass_line_parity_ac_3_2`, and stability test `test_game_golden_vectors_stable_ac_4_2`; JS/TS consumers are in separate repositories (AC-3.2 bindings generated by `export_protocol --typescript`).
- [x] Add v2 golden vectors + size regression tests (payload + state blobs).
  - Tests: AC-1.1, AC-1.2, AC-4.1, AC-4.2
  - Perceptual: None
  - Implemented: Added 10 state blob golden vectors for all games (Blackjack, Baccarat, Roulette, Craps, Sic Bo, Three Card, Ultimate Hold'em, Casino War, Video Poker, Hi-Lo); bumped `GOLDEN_VECTORS_SCHEMA_VERSION` to 3; added size regression tests `test_move_payload_size_reduction_ac_1_1` (verifies >=40% payload reduction vs v1), `test_header_only_moves_1_byte_ac_1_1` (verifies 1-byte header-only moves), `test_roulette_state_size_reduction_ac_1_2` (verifies >=30% state reduction using `estimate_v1_size()`), `test_all_state_blobs_size_reduction_ac_1_2` (verifies all 10 games meet 30% reduction); added parity tests `test_state_blob_vectors_stable_ac_4_2`, `test_roulette_state_roundtrip_ac_4_2`, `test_craps_state_roundtrip_ac_4_2`; all 88 golden vector tests pass.
